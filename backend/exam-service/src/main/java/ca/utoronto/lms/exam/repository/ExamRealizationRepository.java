...
public interface ExamRealizationRepository extends BaseRepository<ExamRealization, Long> {
  ...
  @Query(
      "select x from #{#entityName} x where x.deleted = false and x.subjectEnrollmentId in"
          + " :subjectEnrollmentIds and (cast(x.id as string) like :search or cast(x.score as"
          + " string) like :search) order by x.examTerm.startTime desc")
  ...
  List<ExamRealization> findBySubjectEnrollmentIdInAndDeletedFalse(List<Long> subjectEnrollmentIds);
}
