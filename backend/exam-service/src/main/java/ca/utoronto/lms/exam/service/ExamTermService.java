...
public class ExamTermService extends ExtendedService<ExamTerm, ExamTermDTO, Long> {
  ...
  public List<ExamTermDTO> findByTeacherId(Long id) {
    List<Long> subjectIds =
        subjectFeignClient.getSubjectByTeacherId(id).stream().map(SubjectDTO::getId).toList();

    List<ExamTermDTO> examTerms =
        mapper.toDTO(
            repository.findByExamSubjectIdInAndDeletedFalseOrderByStartTimeDesc(subjectIds));
    return examTerms.isEmpty() ? examTerms : this.mapMissingValues(examTerms);
  }
  ...
  public Page<ExamTermDTO> findByStudentId(Long id, Pageable pageable, String search) {
    ...
    List<Long> subjectIds =
        subjectFeignClient.getSubjectByStudentId(id).stream().map(SubjectDTO::getId).toList();
    ...
    Page<ExamTermDTO> examTerms =
        repository
            .findRegistrableContaining(
                subjectIds, registeredExamTermIds, LocalDate.now(), pageable, "%" + search + "%")
            .map(mapper::toDTO);

    return examTerms.isEmpty()
        ? examTerms
        : new PageImpl<>(
            this.mapMissingValues(examTerms.getContent()), pageable, examTerms.getTotalElements());
  }
}
