...
public interface ExamTermRepository extends BaseRepository<ExamTerm, Long> {
  @Override
  @Query(
      "select x from #{#entityName} x where x.deleted = false and (cast(x.id as string) like"
          + " :search or cast(x.startTime as string) like :search or cast(x.endTime as string) like"
          + " :search)")
  Page<ExamTerm> findContaining(Pageable pageable, String search);

  @Query(
      "select x from #{#entityName} x where x.deleted = false and x.exam.subjectId in :subjectIds"
          + " and not x.id in :registeredExamTermIds and :date between"
          + " x.examPeriod.registrationStartDate and x.examPeriod.registrationEndDate and"
          + " (cast(x.id as string) like :search or cast(x.startTime as string) like :search or"
          + " cast(x.endTime as string) like :search)")
