...
public interface ThesisRepository extends BaseRepository<Thesis, Long> {
  ...
  @Query(
      "select x from #{#entityName} x where x.deleted = false and (cast(x.id as string) like"
          + " :search or x.name like :search or x.description like :search or x.url like :search or"
          + " cast(x.applicationDate as string) like :search or cast (x.defenseDate as string) like"
          + " :search or cast(x.grade as string) like :search)")
