...
public interface SubjectEnrollmentRepository extends BaseRepository<SubjectEnrollment, Long> {
  ...
  @Query(
      "select x from #{#entityName} x where x.deleted = false and (cast(x.id as string) like"
          + " :search or cast(x.extraPoints as string) like :search or cast(x.grade as string) like"
          + " :search)")
  ...
  @Query(
      "select x from #{#entityName} x where x.deleted = false and x.subject.id = :subjectId and"
          + " (cast(x.id as string) like :search or cast(x.extraPoints as string) like :search or"
          + " cast(x.grade as string) like :search)")
