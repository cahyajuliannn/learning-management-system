...
public class SubjectEnrollmentController
  ...
  public ResponseEntity<Page<SubjectEnrollmentDTO>> getBySubjectId(
      @PathVariable Long id, Pageable pageable, @RequestParam(defaultValue = "") String search) {
  ...
  public ResponseEntity<Page<SubjectEnrollmentDTO>> getByStudentId(
      @PathVariable Long id, Pageable pageable, @RequestParam(defaultValue = "") String search) {
    return new ResponseEntity<>(this.service.findByStudentId(id, pageable, search), HttpStatus.OK);
  }
