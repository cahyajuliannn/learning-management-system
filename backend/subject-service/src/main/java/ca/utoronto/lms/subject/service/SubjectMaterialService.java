...
public class SubjectMaterialService
  ...
  public SubjectMaterialDTO save(SubjectMaterialDTO subjectMaterialDTO) {
    if (hasAuthority(ROLE_TEACHER)) {
      ...
      if (!subject.getProfessor().getId().equals(teacher.getId())
        ...
        throw new ForbiddenException("You are not allowed to manager this subject material");
      }
  ...
  public void delete(Set<Long> id) {
    if (hasAuthority(ROLE_TEACHER)) {
      ...
      List<SubjectMaterial> subjectMaterials = (List<SubjectMaterial>) repository.findAllById(id);
      boolean forbidden =
      ...
      if (forbidden) {
        throw new ForbiddenException("You are not allowed to delete these subject materials");
      }
    }
  ...
  public List<SubjectMaterialDTO> findBySubjectId(Long id) {
    ...
    List<SubjectMaterialDTO> subjectMaterials =
        mapper.toDTO(repository.findBySubjectIdAndDeletedFalseOrderByPublicationDateDesc(id));
    return subjectMaterials.isEmpty() ? subjectMaterials : this.mapMissingValues(subjectMaterials);
  }
